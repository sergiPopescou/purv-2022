...
/*************** Statistic Valus **********************/                
volatile int stat_live = 0;
volatile int stat_born = 0;
volatile int stat_dead = 0;
volatile int stat_gen  = 0;
                ...
                struct kobj_attribute *attr, char *buf)
{
        ...
        return sprintf(buf, "Alive: %d\nBorn: %d\nDead: %d\nGen: %d\n", stat_live, stat_born, stat_dead, stat_gen);
}
                ...
                struct kobj_attribute *attr,const char *buf, size_t count)
{
        ...
        sscanf(buf,"%d %d %d %d", &stat_live, &stat_born, &stat_dead, &stat_gen);
        return count;
}
