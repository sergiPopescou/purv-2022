...
int zive_celije = 0, rodjene_celije = 0, mrtve_celije = 0, br_generacija = 0;
...
void num_of_live_cells(int** table){
	for(i = 0; i < ROW_NUM; i++)
        for(j = 0; j < COL_NUM; j++)
            zive_celije += table[i][j];
}
int main (int argc, char *argv[]) {
    ...
    while(1){
		if(getchar() == 's'){
			stop = 0;
			num_of_live_cells(board);
			char statistika[100];
			sprintf(statistika, "Broj generacija: %d\nRodjene celije: %d\nUmrle celije: %d\nZive celije: %d\n", br_generacija, rodjene_celije, mrtve_celije, zive_celije);
			int fd = open("/proc/game_of_life_stat",O_RDWR);
			if(fd < 0) {
				printf("Cannot open device file...\n");
				return 0;
			}
			write(fd,statistika,strlen(statistika));
			close(fd);
