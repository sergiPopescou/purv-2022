...
int32_t value = 0;
char stat_array[100]="Trenutno nema statistike\n\0";
static int len = 1;
...
static ssize_t read_proc(struct file *filp, char __user *buffer, size_t length,loff_t * offset)
{
    ...
    copy_to_user(buffer,stat_array,100);
...
static int __init stat_driver_init(void)
{
        ...
        /*Creating Proc entry*/
        proc_create("game_of_life_stat",0666,NULL,&proc_fops);
...
{
        remove_proc_entry("game_of_life_stat",NULL);
...
MODULE_DESCRIPTION("Modifikacija Game of Life-a");
MODULE_VERSION("1.0");
